...
function FindOnPage(name, status) {
        ...
        if(input.length>=3)
	{
              function FindOnPageGo() {
		        ...
		        result_arr[i] = result[i].replace(eval(search), '<span style="color:#ffffff; background-color:#8200FF; padding: 5px 10px">'+input+'</span>'); //находим нужные элементы, задаем стиль и сохраняем в новый массив
			}
		     for(var i=0; i<result.length;i++) {
